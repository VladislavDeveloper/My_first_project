export default ({
    ...
    mounted() {
        this.$parent.getJson('/api/accessoires')
            .then(data => {
                for (let el of data) {
                    this.accessoires.push(el);
                }
            })
    },
    template: `
    <div class="fetured_items conteiner">
                    <h2 class="feturedTitle">Аксессуары</h2>
                    <div class="products">
                        <product v-for="item of accessoires" :key="item.id_product" :product="item"></product>
                    </div>
    </div>
